...
import { useT, useLocale, roleLabel, currencyLabel, categoryLabel } from '@/i18n';
import { usePinnedCurrencyOptions } from '@/hooks/usePinnedCurrencies';
import type { Category, Locale, Role, User } from '@/types';
...
function PersonalCategoriesSection({ userId }: { userId: string }) {
  const { t } = useT();
...
function HouseholdCategoriesSection({ householdId }: { householdId: string }) {
  const { t } = useT();
