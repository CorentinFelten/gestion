...
/**
 * Record / edit a personal transaction (income · expense · transfer). Extracted
 * from the standalone Add page so the exact same form can render inline in a
 * page (`mode="page"`) or inside an overlay dialog (`mode="modal"`), for a
 * smoother "add without leaving the list" flow.
 *
 * - `page` mode keeps the "saved, add another" banner + soft reset.
 * - `modal` mode calls `onDone` on a successful save (the caller closes the
 *   overlay). Edits always call `onDone` so the caller can dismiss.
 */
export function MoneyTransactionForm({
  ...
  mode: 'page' | 'modal';
  onDone?: () => void;
  /** Preselect the source account for a new transaction (ignored when editing). */
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (editing) {
      ...
      update.mutate({ id: editing.id, input: patch }, { onSuccess: () => onDone?.() });
      return;
    }
  ...
  if (accounts.isLoading) {
    return <LoadingBlock label={t('money.loadingAccounts')} />;
  }
  if (accounts.isError) {
    return (
        ...
        <div className="flex flex-col items-start gap-3 border-t border-gray-100 pt-5 dark:border-gray-800 sm:flex-row sm:items-center">
          ...
          {amountPositive && account ? (
            <span className={tabular('text-sm text-gray-400')}>
              {typeLabel}{' '}
              {f.money(
                isTransfer ? amountInput : (convertedAccountAmount ?? amountInput).toString(),
                accountCurrency,
              )}
            </span>
          ) : null}
