...
3. **Net worth uses the *latest* rate** (current value), the one deliberate exception to the freeze rule (`FxService.getLatestRate`). Account balances are in each account's native currency; aggregation converts to the user's profile currency. The **net-worth trend** (`/me/net-worth/history`) is different: it is *derived* per calendar day from account creation onward (every day filled, even those with no transaction / when the app wasn't running) and values each past day at **that day's own historical rate** via `FxService.getRateSeries`, so the curve reflects real FX movement, not just balance changes.
...
- `backend`: `npm run build` **green** and `npm test` **green** (~175 tests across auth, fx, transactions, settlements, tally, personal, categories, users, households/invites, common).
